...
class Label {
    constructor (id, date, time, anc, qty, from_st, from_bin, to_st, to_bin, tr_order, tr_item, material_description, user, unit) {
        this.id = id;
        this.date = new Date(((date) - 25569) * 86400 * 1000).toLocaleDateString();

        const _date = new Date(((date) - 25569) * 86400 * 1000);
        const _startOfYear = new Date(_date.getFullYear(), 0, 1);
        const _dayOfYear = Math.floor((_date - _startOfYear) / 86400000);
        const _firstDayOfYear = _startOfYear.getDay();
        this.weekNumber = Math.ceil((_dayOfYear - (7 - _firstDayOfYear)) / 7);

        this.time = this.formatedTime(time);
        this.anc = anc.replace(/\d{3}(?!$)/g, "$& ");
        this.qty = qty;
    ...
    init () {
        ...
        <div class="label_container">
            <div class="row">
                ...
                <div class="col">
                    <div class="col p0">
                        <span class="fs-2 fw-5 text-center" style="word-break: break-all;">${this.user}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col flex-100">
                    <div class="col p0">
                        <span class="fs-3 fw-5">TR.ORDER / ITEM</span>
                    </div>
                    <div class="col p0">
                ...
                <div class="col">
                    <div class="col p0">
                        <span class="fs-3 fw-5">WEEK</span>
                    </div>
                    <div class="col p0">
...
function print () {
    const mywindow = window.open('', 'PRINT', "width=840; height=592;");
